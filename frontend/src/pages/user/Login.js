...
function Login() {
  ...
  const loadCaptcha = async () => {
    try {
      ...
      setFormError('Không thể tải CAPTCHA, vui lòng thử lại.');
    } finally {
...
useEffect(() => {
  ...
  const handleLogin = (e) => {
    ...
    // 🔐 CAPTCHA validation - BẮT BUỘC nhập
    if (!captchaInput) {
      setFormError('Vui lòng nhập mã CAPTCHA.');
      return;
    }
            ...
            <Form onSubmit={handleLogin} noValidate>
              ...
              <Form.Group className="mb-3" controlId="loginCaptcha">
                <div className="d-flex align-items-center gap-2">
                  ...
                  >
                    {captchaLoading ? <Spinner size="sm" /> : '🔄'}
                  </Button>
                </div>
